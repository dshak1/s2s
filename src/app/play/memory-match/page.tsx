...
type Tile = { key: string; item: VocabItem; face: "img" | "word" };

function buildBoard(pairs: number): Tile[] {
  const items = sample(VOCAB, pairs);
  const tiles: Tile[] = [];
  for (const item of items) {
    tiles.push({ key: `${item.slug}-img`, item, face: "img" });
    tiles.push({ key: `${item.slug}-word`, item, face: "word" });
  }
...
export default function MemoryMatch() {
  ...
  const [board, setBoard] = useState(() => buildBoard(6));
      ...
      if (a.item.slug === b.item.slug) {
        ...
        if (nm.size === board.length) {
          ...
          store.recordGameRun({
            game: "memory-match",
            score,
            vocabSeen: board.filter((t) => t.face === "img").map((t) => t.item.slug),
            vocabCorrect: board.filter((t) => t.face === "img").map((t) => t.item.slug),
          });
  ...
  return (
    <GameShell title="Esten Qaldyrma" kk="Естен қалдырма" right={<Scoreboard label="Moves" value={moves} />}>
      {done && <Confetti />}
              ...
              >
                ...
                >
                  {tile.face === "img" ? (
                    ...
                    <img src={imgFor(tile.item)} alt={tile.item.en} className="h-full w-full object-contain" />
                  ) : (
